...
def menu():  # David
    ...
    while True:
        for event in pygame.event.get():
            ...
            if np.amax(grid) == 0:  # nr. max. din matrice inainte sa inceapa jocul (cumva)
                if event.type == KEYDOWN:
                    play()
            else:  # reinceprea jocului dupa ce ai pierdut
                if event.type == KEYDOWN:
                    if event.key == K_SPACE:
                        grid = np.zeros((N, N), dtype=int)
                        play()
        if np.amax(grid) != 0:
            text = myfont.render("Apasa SPACE pentru a reincepe", True, CP["textfinish"])
            textpos = text.get_rect()
            textpos.center = (W // 2, H // 2)
            screen.blit(text, textpos)
            scoretext = myfont.render("Cel mai mare numar atins: "+str(np.amax(grid)), True, CP["textfinis"])
            scorepos = scoretext.get_rect()
            scorepos.center = (W // 2, H // 3)
            screen.blit(scoretext, scorepos)
        else:
            text = myfont.render("Apasa ceva", True, CP["textstart"])
            textpos = text.get_rect()
            textpos.center = (W // 2, H // 2)
            screen.blit(text, textpos)
